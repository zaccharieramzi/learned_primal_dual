...
with tf.name_scope('summaries'):
    ...
    test_summary_writer = tf.summary.FileWriter(logs_dir + '/test_{run_id}'.format(run_id=run_id))
    train_summary_writer = tf.summary.FileWriter(logs_dir + '/train_{run_id}'.format(run_id=run_id), sess.graph)
...
# Train the network
for i in tqdm(range(0, maximum_steps)):
    if i%10 == 0:
